...
class ProductGrid extends StatelessWidget {
  ...
  const ProductGrid({super.key, required this.itemCount, required this.isMainTab, required this.width, required this.height, required this.panelProduct, this.onTap, this.onLongPress});
  ...
  Widget build(BuildContext context) {
    ...
    return GridView.builder(
      ...
      gridDelegate: SliverGridDelegateWithMaxCrossAxisExtent(maxCrossAxisExtent: (width * 0.7) / 5, mainAxisExtent: gridHeight / 4, crossAxisSpacing: 8, mainAxisSpacing: 8),
      itemCount: itemCount,
      itemBuilder: (context, index) {
        ...
        final Widget productVisual =
            showType == 'color' && colorHex != null
                ? Container(width: double.infinity, decoration: BoxDecoration(color: hexToColor(colorHex), borderRadius: const BorderRadius.vertical(top: Radius.circular(6))))
                    ...
                    ? ClipRRect(
                      ...
                      child: Image.network(
                        ...
                        loadingBuilder: (context, child, loadingProgress) {
                          ...
                          return Container(color: Colors.grey[200], child: const Center(child: Icon(Icons.image_outlined, size: 40, color: Colors.grey)));
                        },
                        errorBuilder: (context, error, stackTrace) => Container(color: Colors.grey[300], child: const Center(child: Icon(Icons.broken_image_outlined, size: 40, color: Colors.grey))),
                      ),
                    )
                    : Container(
                      width: double.infinity,
                ...
                ? Column(
                  children: [
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          Text(name, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis),
                          const SizedBox(height: 2),
                          Text('฿${product.value?.price ?? 0}', style: const TextStyle(color: Colors.greenAccent, fontSize: 16, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
                        ],
                      ),
                    ),
                  ],
                )
                : Container(decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(6)), child: const Center(child: Icon(Icons.add, color: Colors.white, size: 40)));
